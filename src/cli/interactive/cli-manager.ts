...
import { ApiConfig, DatabaseService, Server, Result, ok, err } from "../../types/index.js";
...
export class InteractiveCliManager {
  ...
  private registerCommands(): void {
    ...
    this.commands.set("restore", async () => await handleRestore(this.database));
    this.commands.set("request", async (args) => await handleRequest(args, this.server));
  ...
  async start(): Promise<Result<void, Error>> {
    try {
      ...
      console.log(OutputFormatter.formatServerOutput(this.config, this.server.getUrl()));
      console.log("\n" + theme.info("Interactive mode enabled. Type 'help' for available commands."));
      ...
      this.rl.on("line", async (line) => {
        ...
        if (handler) {
          ...
          console.log(theme.error(`Unknown command: ${command}. Type 'help' for available commands.`));
        }
  ...
  async exit(): Promise<void> {
    ...
    if (stopResult.ok === false) {
      console.error(theme.error("Error stopping server:"), stopResult.error.message);
      process.exit(1);
    }
