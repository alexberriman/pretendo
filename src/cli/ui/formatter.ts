...
export class OutputFormatter {
  ...
  static formatServerOutput(config: ApiConfig, serverUrl: string): string {
    ...
    output.push(formatKeyValue("Page Size", `${defaultPageSize} (max: ${maxPageSize})`));
    ...
    if (errorSimulation?.enabled) {
      output.push(formatKeyValue("Error Rate", `${(errorSimulation.rate || 0) * 100}%`));
    }
    ...
    config.resources.forEach((resource) => {
      ...
      output.push(formatEndpoint("GET", `/${resourceName}`, `List all ${resourceName}`));
      output.push(formatEndpoint("GET", `/${resourceName}/:id`, `Get a single ${resourceName}`));
      output.push(formatEndpoint("POST", `/${resourceName}`, `Create a new ${resourceName}`));
      output.push(formatEndpoint("PUT", `/${resourceName}/:id`, `Update a ${resourceName}`));
      output.push(formatEndpoint("PATCH", `/${resourceName}/:id`, `Partially update a ${resourceName}`));
      output.push(formatEndpoint("DELETE", `/${resourceName}/:id`, `Delete a ${resourceName}`));
    ...
    output.push(formatEndpoint("POST", "/__reset", "Reset database to initial state"));
  ...
  static formatDatabaseStats(database: any): string {
    const output: string[] = [];
    ...
    Object.entries(stats).forEach(([resource, data]: [string, any]) => {
      table.push([
        theme.text(resource),
  ...
  static formatHelpMenu(): string {
    ...
    table.push(
      [theme.command("help"), theme.text("Show this help menu")],
      [theme.command("routes"), theme.text("List all API routes")],
      [theme.command("config"), theme.text("Show current configuration")],
      [theme.command("stats"), theme.text("Show database statistics")],
      [theme.command("reset"), theme.text("Reset the database to initial state")],
      [theme.command("backup"), theme.text("Create a database backup")],
      [theme.command("restore"), theme.text("Restore from the last backup")],
      [theme.command("request"), theme.text("Make a test request to an endpoint")],
      [theme.command("clear"), theme.text("Clear the console")],
      [theme.command("exit"), theme.text("Stop the server and exit")]
    );
