...
export const handleConfig = async (config: ApiConfig): Promise<void> => {
  ...
  const table = new Table({
    head: [theme.heading("Option"), theme.heading("Value")],
    style: {
      head: [],
      border: [],
    },
  ...
  if (auth?.enabled) {
    table.push(["Authentication", theme.success("Enabled")]);

    if (auth.authEndpoint) {
      table.push(["Auth Endpoint", theme.text(auth.authEndpoint)]);
    }
  ...
  Object.entries(otherOptions).forEach(([key, value]) => {
    if (typeof value === "object") {
      table.push([key, theme.dimText(JSON.stringify(value))]);
    } else {
      table.push([key, theme.text(String(value))]);
    }
  });
