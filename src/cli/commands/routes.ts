...
export const handleRoutes = async (config: ApiConfig): Promise<void> => {
  ...
  config.resources.forEach((resource) => {
    ...
    table.push(
      [
        theme.get("GET"),
        theme.text(`/${resourceName}`),
        theme.dimText(`List all ${resourceName}`),
      ],
      [
        theme.get("GET"),
        theme.text(`/${resourceName}/:id`),
        theme.dimText(`Get a single ${resourceName}`),
      ],
      [
        theme.post("POST"),
        theme.text(`/${resourceName}`),
        theme.dimText(`Create a new ${resourceName}`),
      ],
      [
        theme.put("PUT"),
        theme.text(`/${resourceName}/:id`),
        theme.dimText(`Update a ${resourceName}`),
      ],
      [
        theme.patch("PATCH"),
        theme.text(`/${resourceName}/:id`),
        theme.dimText(`Partially update a ${resourceName}`),
      ],
      [
        theme.delete("DELETE"),
        theme.text(`/${resourceName}/:id`),
        theme.dimText(`Delete a ${resourceName}`),
      ],
    );
  ...
  adminTable.push(
    [
      theme.post("POST"),
      theme.text("/__reset"),
      theme.dimText("Reset database to initial state"),
    ],
    [
      theme.post("POST"),
      theme.text("/__backup"),
      theme.dimText("Create database backup"),
    ],
    [
      theme.post("POST"),
      theme.text("/__restore"),
      theme.dimText("Restore from backup"),
    ],
  );
